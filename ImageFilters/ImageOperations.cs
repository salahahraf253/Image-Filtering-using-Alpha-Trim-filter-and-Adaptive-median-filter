...
namespace ImageFilters
{
    public class ImageOperations
    {
            ...
            try
            {
                ...
                for (int y = 0; y < Height; y++)
                {
                    for (int x = 0; x < Width; x++)
                    {
                        ...
                        {
                            newImage[y, x] = Alpha_Trim_Filter.appplyFilterofAlphaTrim(ImageMatrix, x, y, Max_Size, sortType, K);
                        }
                        else if (filterType == 2)  //user chooses adaptive medain filter
                        {
                            newImage[y, x] = Adaptive_medain_filter.applyFilterOfMedain(ImageMatrix, x, y, Max_Size, sortType);
                        }
                        else
                        {
        ...
        {
            byte[] windowOfPixels = new byte[windowSize * windowSize];
            int yPos, xPos;
            ...
            for (int i = 0; i < windowSize; i++)
            {
                ...
                {
                    windowOfPixels[windowPixelSize++] = ImageMatrix[yPos, xPos];
                }
            }
            return windowOfPixels; //return window (invalid neighbour is zero)
        }
